...
const Games = () => {
  ...
  return (
    <div className="bg-games-bg bg-cover h-screen flex flex-col gap-4 2xl:gap-8 justify-center items-center font-fira-sans">
      ...
      {isTabletOrMobile ? (
        websites &&
        websites.games.slice(0, slice).map((i, index) => (
          <div
            key={index}
            className="md:h-96 md:w-[800px] w-52 h-96 2xl:h-2/3 2xl:w-3/4 bg-pastel-bugday items-center z-10 shadow-extra border-siyah-text border-4 flex"
          >
            <div className="grid grid-cols-1 px-4 py-10 gap-10 w-full md:-translate-y-4 2xl:translate-y-0">
              {/* Games */}
          ...
          <div className="grid grid-cols-3 px-6 py-10 gap-10 w-full md:-translate-y-4 2xl:translate-y-0 ">
            ...
            {websites &&
              websites.games.slice(0, slice).map((i, index) => (
                <div
                  key={index}
                  className="rounded-md bg-pastel-yesil flex flex-col items-center 2xl:py-20  2xl:gap-10 gap-4 shadow-md justify-center py-4 h-fit "
                >
                  <img
                    alt={i.title}
                    src={i.img}
                    className="w-40 h-40 2xl:w-60 2xl:h-60 object-cover object-top "
                  />
                  <div className="text-center flex flex-col 2xl:gap-2 gap-0">
                    <h1 className="text-white 2xl:text-3xl md:text-xl font-semibold ">
                      {i.title}
